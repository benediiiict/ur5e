...
class PIHAssembleEnv(gym.Env):
  ...
  def __init__(self):
    ...
    self.position_ready = np.array([0.5522, 0.1600, 0.3528]) 
    self.pose_last = []
    self.distance = []
...
########################################### Stablebaseline3 ##################################################
  ...
  def step(self, action):
    ...
    if self.pose.position.x > (self.target[0] - 0.0035) and self.pose.position.x < (self.target[0] + 0.0025) and self.pose.position.z > (self.target[2] + 0.015) and self.pose.position.z < (self.target[2] + 0.025):
      reward = 3
      done = True
      ...
      if self.force[0] > 15 or self.force[0] < -15 :
        ...
        #   reward += abs(self.pose.position.z - self.pose_last.position.z) * 1000
        ...
        # elif self.pose.position.z > (self.target[2] + 0.015) and self.pose.position.z < (self.target[2] + 0.025):
        #   reward += 1
        # elif self.pose.position.z < (self.target[2] + 0.015) or self.pose.position.z > (self.target[2] + 0.125):
        #   reward -= 3
        #   done = True
        ...
        if (self.target[0] + 0.0025) > self.pose.position.x > (self.target[0] - 0.0035):
          reward += 1.0
        elif (self.target[0] + 0.0060) > self.pose.position.x > (self.target[0] + 0.0025):
          reward += 0.5
        elif (self.target[0] + 0.0100) > self.pose.position.x > (self.target[0] + 0.0060):
          reward += 0.2
        elif (self.target[0] - 0.0035) > self.pose.position.x > (self.target[0] - 0.0055):
          reward += 0.5  
        elif (self.target[0] - 0.0055) > self.pose.position.x > (self.target[0] - 0.0080):
          reward += 0.2
    ...
    # if 0.555 > self.pose.position.x > 0.550 and 0.1600 > self.pose.position.y > 0.1590 and 0.2780 > self.pose.position.z > 0.2700:
